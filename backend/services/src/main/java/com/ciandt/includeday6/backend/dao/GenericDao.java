...
import static com.ciandt.includeday6.backend.util.OfyService.ofy;
...
public class GenericDao<T> implements IGenericDao<T> {
    ...
    @Override
    public List<T> listByFilterWithOrder(Filter filtro, String orderField) {
        return ofy().load().type(clazz).filter(filtro).order(orderField).list();
    }
    ...
    public List<T> listByPropertyWithOrder(String propName, Object propValue,String orderField) {
        return ofy().load().type(clazz).filter(propName, propValue).order(orderField).list();
    }
}